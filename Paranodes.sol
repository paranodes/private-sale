...
contract NODERewardManagement {
        ...
        returns (uint256 availableRewards)
    {
        ...
        return
            minutesPassed.mul(rewardsPerMinute).add(nodeStartAmount).sub(
                node.dividendsPaid
            );
    }
    ...
    function getNodes(address user)
        external
        view
...
contract Paranodes is ERC20, Ownable, PaymentSplitter {
    ...
    uint256 public maxTxAmount = 100 * 10**18;
    ...
    {
        require(
            bytes(prefix).length > 3 && bytes(prefix).length < 28,
            "NODE CREATION: PREFIX SIZE INVALID"
        );
        address sender = msg.sender;
        uint256 nodePrice = nodeRewardManagement.nodePrice();
        require(
